...
namespace FEA2D.Structures
{
    public class StructureIO
    {
        ...
        public void OnCreate()
        {
            ...
            string material = "CREATE TABLE  if not exists material(id integer primary key autoincrement, e number, u number, label text, alpha number, gamma number);";
            ...
            string supports = "CREATE TABLE  if not exists supports(id integer primary key autoincrement, node_label text, restrainx number,restrainy number,restrainR number); ";
            string pointloads = "CREATE TABLE  if not exists pointloads(id integer primary key autoincrement, node_label text, magnitude number, angle number); ";
        ...
        public void OnUpgrade()
        {
            ...
            string pointloads = "DROP TABLE  if exists pointloads; ";
            command = new SqliteCommand(material, db);
            command.ExecuteNonQuery();
            command = new SqliteCommand(section, db);
            command.ExecuteNonQuery();
        ...
        public void Save(Structure structure)
        {
            ...
            string pointloads = "DELETE FROM pointloads; ";
            command = new SqliteCommand(material, db);
            command.ExecuteNonQuery();
            command = new SqliteCommand(section, db);
            command.ExecuteNonQuery();
            ...
            foreach (FrameElement2D element in structure.Elements)
            {
                ...
                command.CommandText = "INSERT INTO material(e,u,label,alpha,gamma) VALUES (@e,@u,@label,@alpha,@gama)";
                ...
                command.CommandText = "INSERT INTO section(x,y,label) VALUES (@x,@y,@label)";
            ...
            foreach (Node2D node in structure.Nodes)
            {
                ...
                if (!node.IsFree)
                {
                    ...
                    {
                        command.CommandText = "INSERT INTO pointloads(node_label,magnitude,angle) VALUES (@node_label,@magnitude,@angle)";
