...
namespace FEA2D.Elements
{
	...
	public partial class FrameElement2D
	{
		...
		public void DrawTrapezoidalLoad(SKCanvas canvas, APG_FEA2D.Views.Grid _grid)
		{
			float pixalDisplay = 50;
			this.paint.Color = SKColors.Yellow;
			for (int i = 0; i < this.Loads.Count; i++)
			{
				if (this.Loads[i] is FrameTrapezoidalLoad load)
				{
					double forceValue1 = 0;
					double forceValue2 = 0;
					double maxForceInElement = Math.Max(Math.Abs(load.Wy1), Math.Abs(load.Wy2));
					double slopeAngle = Math.Atan2((this.EndNode.Y - this.StartNode.Y), (this.EndNode.X - this.StartNode.X));
					double elementPerpendicularSlopeAngle = Math.Atan(-1 / Math.Tan(slopeAngle));
					Point segmentStartPointInPixel = _grid.RealDisplayCoord(new Point(this.StartNode.X, this.StartNode.Y));
					double delL = this.Length / 5 * _grid.spacing / APG_FEA2D.Views.Grid.SpacingEquivalentInGrid;
					Point delPointChange = new Point(delL * Math.Cos(slopeAngle), -delL * Math.Sin(slopeAngle));
					for (int j = 0; j < 5; j++)
					{
						forceValue1 = load.Wy1 + (load.Wy2 - load.Wy1) / 5 * j;
						forceValue2 = load.Wy1 + (load.Wy2 - load.Wy1) / 5 * (j + 1);
						double scaledFx1 = forceValue1 / maxForceInElement * pixalDisplay;
						double scaledFx2 = forceValue2 / maxForceInElement * pixalDisplay;
						...
						//string textstart = $"{load.Wy1:f3}";
						//canvas.DrawText(textstart, (float)forceEndPointInPixel.X, (float)forceEndPointInPixel.Y, paint);
						//string textend = $"{load.Wy1:f3}";
						//canvas.DrawText(textend, (float)forceEndPointInPixel.X, (float)forceEndPointInPixel.Y, paint);
					}
				}
			}
